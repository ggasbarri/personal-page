...
- **iOS, Liquid Glass.** A real glass *material*, built in two tiers so it is correct on every engine and refractive where it can be.
  ...
  - **Refraction (Chromium only):** an SVG displacement filter (`#lq-glass`, a convex-lens map with a subtle chromatic edge) is layered into `backdrop-filter` via `url("#lq-glass")`. SVG-in-`backdrop-filter` is a Chromium-only capability, so `main.js` gates it behind `html.refract` (UA + `CSS.supports` check). Safari/Firefox simply keep the frosted base; the refraction never breaks them.
