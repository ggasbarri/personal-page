...
- **iOS, Liquid Glass.** A real glass *material*, built in two tiers so it is correct on every engine and refractive where it can be.
  ...
  - **Where it applies (capability-detected, no UA sniff):** Chromium supports an SVG filter inside `backdrop-filter`, so `--lq` is routed there (`html.refract`) to bend the **live backdrop** — the real deal. Safari/Firefox don't support `url()` in `backdrop-filter`, so the same `--lq` is **attempted** through element `filter` (`html.refract-el`) at conservative strength; where the engine cooperates it warps the surface, where it doesn't the frosted base stands. If `glass.js` never runs, `var(--lq, none)` collapses to `none` and every surface keeps the frosted base. The refraction never breaks any engine.
