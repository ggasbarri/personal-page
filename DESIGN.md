...
- **App icons** carry per-app committed colors (Maps blue, Ledger green, Terminal phosphor-dark, Mail sky, Notes yellow, Settings gray; Messages keeps the seed accent) — fixed, not seed-derived, so the home grid reads like a real springboard. A shared **mini-tile family** (`.minitile`: glyph on committed color, squircle on iOS / circle on Android) carries the same icons into the lock-screen notifications, the Live Activity checklist, and the home progress widget.
...
- **Live Activity spine.** Outside Messages the LA is the **discovery tracker**: ring/bar fills as apps are visited ("3/6 explored"); expand card shows a per-app checklist (mini tiles, visited in full color, unvisited grayscale ghosts) whose footer becomes a real "notes unlocked" button that launches Notes once all six are open. Inside Messages the LA reverts to the story-beat spine (per-beat `activity` state from `data.js`), then restores to discovery on close. It should read as "work in motion" or "clear path", not as one specific workstream.
